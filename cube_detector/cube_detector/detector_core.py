...
############
# Core algorithm for cube_detector package.
# Can be called from a python script for testing or by a ros node wrapper for live use.
# Main functions are findCubePoses and findCubePosesCanny. findCubePosesCanny uses canny edge detection
# for finding cubes and findCubePoses uses hsv masking for edge detection.
############
...
def findCubePosesCanny(image: np.ndarray, colors: dict, K: np.ndarray, D: np.ndarray, params: dict):
    # Setup - Load parameters
    cubeSize     = params.get('cubeSize', 0.05) # Size of cube
    minArea      = params.get('minArea', 2500) # Minimum area of contour
    maxArea      = params.get('maxArea', 30000) # Maximum area of contour
    drawColor = params.get('drawColor', True) # Draw color estimate
    cannyMin = params.get('cannyMin', 100) # Canny minimum threshold value
    cannyMax = params.get('cannyMax', 200) # Canny maximum threshold value
    alpha        = params.get('alpha', .05) # Epsilon factor, percent of arc length
    morphOpen   = params.get('morphOpen', 3)
    morphClose  = params.get('morphClose', 5)
    reprojMax   = params.get('reprojMax', 8.0) # Max reprojection error
    enableBlur   = params.get('enableBlur', True) # Enable gaussian blur
    blurKernelSize = params.get('blurKernelSize', 5) # Kernel size for gaussian blur
    displayCannyMask = params.get('displayCannyMask', False) # Display only canny mask
    preprocessing = params.get('preprocessing', True) # Preproccessing of image using clahe
    claheClipLimit = params.get('claheClip', 2.0) # Clahe clip limit
    claheTileSize = params.get('claheTile', 8) # Clahe tile size
    enableMaskCleanup = params.get('maskCleanup', True) # Enable morphology opening and closing on color mask before finding contours
    ...
    mask = cv.Canny(gray, cannyMin, cannyMax)
    # Mask cleanup before finding contours
    if enableMaskCleanup:
        kernel_open  = cv.getStructuringElement(cv.MORPH_RECT, (morphOpen,morphOpen))
        kernel_close = cv.getStructuringElement(cv.MORPH_RECT, (morphClose,morphClose))
    ...
    for cnt in cnts:
        ...
        name = FindContourColor(overlay, cnt, colors)
        if name == '': continue

        # Draw guessed color on contour
        if drawColor:
            overlay = DrawColorName(overlay, cnt, name)
...
def DrawContours(image, cnts, min, max, shapeCheck):
    ...
    for cnt in cnts:
        ...
        if shapeCheck:
            ...
            extent = area / float(w*h)
            aspect = w / float(h)
            if (min < area < max) and (solidity > 0.9) and (extent > 0.70) and (0.6 < aspect < 1.4):
...
def FindContourColor(image, cnt, colors: dict):
    hsv = cv.cvtColor(image, cv.COLOR_BGR2HSV)
    mask = np.zeros(image.shape[:2], np.uint8)
    cv.drawContours(mask,[cnt],-1,(255,255,255),-1)
    h, s, v, _ = cv.mean(hsv, mask=mask)
    for name, color in colors.items():
        hmin, smin, vmin = color[0]
        hmax, smax, vmax = color[1]
        if (hmin <= h <= hmax) and (smin <= s <= smax) and (vmin <= v <= vmax):
            return name
    return ''

def DrawColorName(image, cnt, name):
    M = cv.moments(cnt)
    cntx = int(M['m10'] / M['m00']) # calculate X position
    cnty = int(M['m01'] / M['m00']) # calculate Y position
    cv.circle(image, (cntx, cnty), 7, (255,0,0), -1)
    cv.putText(image, name, (cntx, cnty), cv.FONT_HERSHEY_SIMPLEX, 1, (255,255,255))
    return image
